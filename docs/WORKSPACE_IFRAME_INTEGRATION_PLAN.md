# Workspace Iframe Integration Plan (v3 — final)
...
- Shows a loading spinner/skeleton inside the iframe container until `READY` is received from the iframe (first load takes a few seconds while bars are fetched from Polygon and the chart engine initializes)
...
## Step Zero: Verify iframe is allowed (Charting Platform)

**Do this FIRST before writing any code.** If nginx or Express sets `X-Frame-Options: DENY` or `X-Frame-Options: SAMEORIGIN`, the browser will refuse to render `charts.theintraday.com` inside an iframe on `theintraday.com` — they are different origins (different subdomains).

1. Check nginx config on EC2 for any `X-Frame-Options` or `Content-Security-Policy` headers
2. Check Express middleware for `helmet()` or manual header setting
3. Set the CSP `frame-ancestors` header (supersedes `X-Frame-Options`) on `/tos/:ticker` responses when `?embed=true`:
   ```
...
5. If `X-Frame-Options` is set at the nginx level, it must be removed or overridden there — a code-level change alone won't help

This can be set conditionally in `tos.ts` when `?embed=true` is present (so standalone `/tos/AAPL` behavior is unchanged), or configured in nginx for the `/tos/` location block.
...
3. **Hide "View on The Intraday" link in embed mode** (user is already on The Intraday)

4. **Hide "Sign In" button in embed mode** (auth is handled by Fin Quote's navbar)

5. **Hide charting app's theme toggle in embed mode** (Fin Quote owns theme control via postMessage)

**Note:** Do NOT send the READY message from `html.ts`. See the dedicated `generateEmbedReadyScript` section below.

### File: `src/server/routes/tos.ts` — READY script placement

**What changes:**

Add a new `generateEmbedReadyScript(ctx)` function that is called **last** in the page assembly, after all other `generate*Script(ctx)` calls. This ensures all modules (including `fundamentals.ts` which defines `setFundWorkspaceMode()`) are fully initialized before READY is sent.

```
function generateEmbedReadyScript(ctx) {
  if (!ctx.embed) return '';
  return `<script>
...
Place this at the very end of the HTML body assembly in the route handler. This guarantees that when Fin Quote receives READY and sends `SET_WORKSPACE_MODE`, the handler function `setFundWorkspaceMode()` is already defined.
...
| File | Action | Description |
|------|--------|-------------|
| nginx config or `src/server/routes/tos.ts` | Modify | Set `Content-Security-Policy: frame-ancestors` header to allow iframe embedding from theintraday.com **(do first)** |
| `src/embed/protocol.ts` | Modify | Add SET_WORKSPACE_MODE message type |
| `src/server/routes/tos.ts` | Modify | Accept `embed`, `view`, `theme` query params, update TosPageContext type, pass to context, add `generateEmbedReadyScript(ctx)` called last in page assembly |
| `src/server/routes/tos-ui/fundamentals.ts` | Modify | Set initial workspace mode from query param |
| `src/server/routes/tos-ui/html.ts` | Modify | Render workspace-switch with display:none in embed mode, add postMessage listener with origin validation, hide Sign In / theme toggle / "View on The Intraday" in embed mode |
...
### Step Zero: Verify iframe headers (do before anything else)

1. Start the charting app locally: `npm run dev`
2. Open browser dev tools → Network tab
3. Visit `http://localhost:3000/tos/AAPL?embed=true`
4. Check the response headers — verify there is NO `X-Frame-Options: DENY` or `SAMEORIGIN`
5. Verify `Content-Security-Policy` includes `frame-ancestors 'self' https://theintraday.com` (or is absent)
6. If blocked: fix nginx config or Express middleware before proceeding

### Testing the Charting Platform changes
