...
export const companyMappings: Record<string, CompanyMapping> = {
  // Tech Giants
  AAPL: {
    symbol: 'AAPL',
    primary: 'Apple Inc',
...
export const getAllTickers = (): string[] => Object.keys(companyMappings)
...
export const findTickerByName = (name: string): string | undefined => {
  ...
  for (const [ticker, mapping] of Object.entries(companyMappings)) {
    if (mapping.primary.toLowerCase() === normalizedName) return ticker

    for (const alias of mapping.aliases) {
      if (alias.toLowerCase() === normalizedName) return ticker
    }
  }
...
export const findTickerByExecutive = (executiveName: string): string | undefined => {
  const normalized = executiveName.toLowerCase().trim()

  for (const [ticker, mapping] of Object.entries(companyMappings)) {
    for (const exec of mapping.executives) {
      if (exec.toLowerCase() === normalized) return ticker
    }
  }
...
export const findTickerByProduct = (productName: string): string | undefined => {
  const normalized = productName.toLowerCase().trim()

  for (const [ticker, mapping] of Object.entries(companyMappings)) {
    for (const product of mapping.products) {
      if (product.toLowerCase() === normalized) return ticker
    }
  }
