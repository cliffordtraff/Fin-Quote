...
import { EarningsContext, ConfidenceInput } from '@watchlist/types/earnings'
import {
  EARNINGS_SCORING_CONFIG,
  EARNINGS_KEYWORDS,
  CONFIDENCE_THRESHOLDS
} from '@watchlist/config/earnings-scoring'
...
export class EarningsContextCalculator {
  ...
  private calculateTemporalScore(
    earningsContext: ConfidenceInput['earningsContext'],
    config: typeof EARNINGS_SCORING_CONFIG
  ): number {
    ...
    switch (status) {
      ...
      case 'recent':
        if (daysSince === 1) return config.temporal.t_plus_1
        if (daysSince === 2) return config.temporal.t_plus_2
        if (daysSince && daysSince >= 3 && daysSince <= 5) return config.temporal.t_plus_3_to_5
        if (daysSince && daysSince >= 6 && daysSince <= 7) return config.temporal.t_plus_6_to_7
        return 0
      case 'upcoming':
        if (daysAway === 1) return config.temporal.t_minus_1
        if (daysAway && daysAway >= 2 && daysAway <= 5) return config.temporal.t_minus_2_to_5
        return 0
      default:
        return 0
    }
  ...
  private calculateVolumeScore(
    quote: ConfidenceInput['quote'],
    config: typeof EARNINGS_SCORING_CONFIG
  ): number {
    if (!quote.volume || !quote.avgVolume) return 0
  ...
  private calculateNewsScore(
    headlines: ConfidenceInput['headlines'],
    config: typeof EARNINGS_SCORING_CONFIG
  ): number {
    const earningsMentions = headlines.filter(h =>
      this.containsEarningsKeywords(h.title) ||
      this.containsEarningsKeywords(h.description || '')
    ).length
  ...
  private calculateGapScore(
    ...
    if (!quote.dayOpen || !quote.previousClose) return 0
   ...
   */
  private containsEarningsKeywords(text: string): boolean {
