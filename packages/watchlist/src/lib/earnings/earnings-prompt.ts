 ...
 */
export function buildEarningsPromptSection(
  ...
  if (status === 'today_bmo' || status === 'today_amc') {
    ...
    if (lastEarnings && lastEarnings.epsActual !== null) {
      ...
      // Pre-report
      section += `- Estimates: EPS ${formatNumber(lastEarnings?.epsEstimate)}, Revenue ${formatRevenue(lastEarnings?.revenueEstimate)}\n`
 ...
 */
function formatEarningsResult(earnings: EarningsData, beatQuality: any): string {
  let result = ''
