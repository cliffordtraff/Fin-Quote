...
export class BeatQualityScorer {
  ...
  calculateBeatQuality(earnings: EarningsData): EarningsBeatQuality {
    const epsQuality = this.assessMetricQuality(
      earnings.epsActual,
      earnings.epsEstimate,
      'EPS'
    )

    const revenueQuality = this.assessMetricQuality(
      earnings.revenueActual,
      earnings.revenueEstimate,
      'Revenue'
    )

    const epsBeatPercent = this.calculateBeatPercent(
      earnings.epsActual,
      earnings.epsEstimate
    )

    const revenueBeatPercent = this.calculateBeatPercent(
      earnings.revenueActual,
      earnings.revenueEstimate
    )
    ...
    const overallScore = this.calculateOverallScore(
      ...
      earnings.revenueActual !== null && earnings.revenueEstimate !== null
    )
  ...
  private assessMetricQuality(
    ...
    metricName: string
  ): { quality: EarningsBeatQuality['epsQuality']; score: number } {
    // Missing data
    ...
    if (estimate === 0) {
      if (actual > 0) return { quality: 'strong_beat', score: 100 }
      if (actual < 0) return { quality: 'strong_miss', score: 0 }
  ...
  private calculateBeatPercent(
    ...
    if (estimate === 0) return actual > 0 ? 100 : -100
  ...
  private calculateOverallScore(
    epsResult: { quality: string; score: number },
    revenueResult: { quality: string; score: number },
    hasEpsData: boolean,
    ...
    if (hasEpsData && hasRevenueData) {
      return Math.round(
        epsResult.score * BEAT_QUALITY_WEIGHTS.EPS +
