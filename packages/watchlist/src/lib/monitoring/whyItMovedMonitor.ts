...
export interface AggregatedMetrics {
  ...
  // Success rates
  validationPassRate: number  // % of requests that passed validation first try
  retryRate: number           // % of requests that required retry
  fallbackRate: number        // % of requests that used fallback
...
const metricsLog: SummaryMetrics[] = []
const MAX_LOG_SIZE = 10000  // Keep last 10k requests

/**
 * Log a summary request with all metrics
 */
export function logSummaryMetrics(metrics: SummaryMetrics): void {
  ...
  console.log(`[AI Summary] ${status}`, {
    ...
    errors: metrics.validationErrors?.length || 0
  })
 ...
 */
export function getAggregatedMetrics(windowMs: number = MONITORING_WINDOW_MS): AggregatedMetrics {
  const now = Date.now()
  ...
  const promptVersion = windowMetrics[windowMetrics.length - 1]?.promptVersion || 'N/A'
 ...
 */
export function checkRollbackTriggers(): RollbackTrigger[] {
 ...
 */
export function getRecentFailures(limit: number = 10): SummaryMetrics[] {
  return metricsLog
 ...
 */
export function exportMetrics(): SummaryMetrics[] {
  return [...metricsLog]
}
