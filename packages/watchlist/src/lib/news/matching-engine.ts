import { NewsArticle, TickerMatch } from '@watchlist/types'
...
export class NewsMatchingEngine {
  ...
  private containsExactTicker(text: string, ticker: string): boolean {
  ...
  private extractUrlSlug(url: string): string[] {
    // Extract potential company/ticker references from URL
    const slugs: string[] = []

    // Remove protocol and domain
    const pathMatch = url.match(/^https?:\/\/[^\/]+(.*)$/)
    if (!pathMatch) return slugs
  ...
  private checkUrlForCompany(url: string, ticker: string, companyName: string): boolean {
  ...
  private findExactMatches(text: string, terms: string[]): string[] {
  ...
  private calculateContextBoost(text: string, mapping: typeof companyMappings[string]): number {
    let boost = 0
    const normalizedText = this.normalizeText(text)
  ...
  private matchArticleToTicker(
    ...
    let confidence = 0
    let matchType: MatchResult['matchType'] = 'context'
    ...
    if (AMBIGUOUS_TICKERS.has(ticker) && confidence > 0) {
      ...
      if (AMBIGUOUS_WITH_CLEAR_NAME.has(ticker)) {
        ...
        const hasCompanyName = new RegExp(
          `\\b${this.normalizeText(mapping.primary).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`,
          'i'
        ).test(this.normalizeText(combinedText))
        ...
        const hasHeadlineMatch =
          this.containsExactTicker(headline, ticker) ||
    ...
    if (confidence >= CONFIDENCE_THRESHOLD) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `[NewsMatching] ${ticker} ← "${article.headline.substring(0, 60)}..." | ` +
    ...
    if (process.env.NODE_ENV === 'development' && confidence > 0 && confidence < CONFIDENCE_THRESHOLD) {
      console.log(
        `[NewsMatching] REJECTED ${ticker} (confidence ${confidence} < ${CONFIDENCE_THRESHOLD}) ← ` +
  ...
  public matchArticles(
    articles: NewsArticle[],
    tickers: string[]
  ): Map<string, NewsArticle[]> {
    const tickerNewsMap = new Map<string, NewsArticle[]>()
    ...
    for (const article of recentArticles) {
      ...
      if (matches.length > 0) {
        ...
        const articleWithMatches = { ...article, matchedTickers: matches }
    ...
    for (const [ticker, news] of tickerNewsMap.entries()) {
      news.sort((a, b) => {
        ...
        const aConfidence = a.matchedTickers?.find(m => m.symbol === ticker)?.confidence || 0
        const bConfidence = b.matchedTickers?.find(m => m.symbol === ticker)?.confidence || 0
  ...
  public deduplicateArticles(articles: NewsArticle[]): NewsArticle[] {
    ...
    const seenTitles = new Map<string, NewsArticle>()
  ...
  public rankArticles(articles: NewsArticle[]): NewsArticle[] {
    const now = Date.now()
    ...
    return articles.map(article => {
      ...
      return { ...article, confidence: Math.max(0, Math.min(100, score)) }
