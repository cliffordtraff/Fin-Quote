import { WatchlistTab, WatchlistStock } from '@watchlist/types';

// Primary exchange mappings for common US stocks
const EXCHANGE_MAPPINGS: Record<string, string> = {
...
function getDefaultExchange(symbol: string): string {
...
function needsMigration(item: any): boolean {
  return item.type === 'stock' && item.symbol && !item.tvSymbol;
}

// Migrate a single watchlist tab
export function migrateWatchlistTabWithTvSymbols(tab: WatchlistTab): WatchlistTab {
  if (!tab.items) {
    return tab;
  }
  ...
  const migratedItems = tab.items.map(item => {
    if (needsMigration(item)) {
      ...
      return {
        ...stock,
        tvSymbol: `${exchange}:${stock.symbol.toUpperCase()}`,
        exchange: exchange
      };
    }
    return item;
  ...
  return hasChanges ? { ...tab, items: migratedItems } : tab;
}

// Migrate all tabs in a watchlist
export function migrateWatchlistWithTvSymbols(tabs: WatchlistTab[]): { 
  tabs: WatchlistTab[];
  migrated: boolean;
} {
...
export function watchlistNeedsMigration(tabs: WatchlistTab[]): boolean {
  return tabs.some(tab => 
    tab.items?.some(item => needsMigration(item)) || false
  );
}