...
function getEasternTime(date: Date): { hours: number; minutes: number; day: number } {
...
export function getExtendedHoursSession(): 'pre-market' | 'after-hours' | null {
  if (!isExtendedHours()) return null
...
export function getCacheMaxAge(dataType: 'quote' | 'dividend' | 'static'): number {
  switch (dataType) {
    case 'quote':
