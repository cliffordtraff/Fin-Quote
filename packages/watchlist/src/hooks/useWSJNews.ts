...
export function useWSJNews({
  symbols,
  enabled = true,
  feedType = 'markets',
  pollInterval = 5 * 60 * 1000 // 5 minutes
}: UseWSJNewsOptions) {
  const [state, setState] = useState<WSJNewsState>({
    news: {},
    loading: false,
    error: null,
    lastUpdated: null
    ...
    try {
      ...
      if (cached) {
        ...
        if (cacheAge < CACHE_TTL) {
          ...
          const filteredNews: Record<string, NewsArticle[]> = {}
          for (const symbol of symbols) {
            if (parsed.news[symbol]) {
              // Parse dates back to Date objects
              filteredNews[symbol] = parsed.news[symbol].map((article: any) => ({
                ...article,
  ...
  const saveToCache = useCallback((news: Record<string, NewsArticle[]>, stats: any) => {
    try {
      const cacheData = {
        news,
        stats,
  ...
  const fetchNews = useCallback(async () => {
    ...
    try {
      ...
      const parsedNews: Record<string, NewsArticle[]> = {}
      for (const [ticker, articles] of Object.entries(data.news || {})) {
        parsedNews[ticker] = (articles as any[]).map(article => ({
          ...article,
          publishedAt: new Date(article.publishedAt)
        }))
      }
  ...
  // Polling
  useEffect(() => {
    if (!enabled || symbols.length === 0 || pollInterval <= 0) return
  ...
  const getNewsForTicker = useCallback((ticker: string): NewsArticle | null => {
  ...
  const getAllNewsForTicker = useCallback((ticker: string): NewsArticle[] => {
    return state.news[ticker] || []
  }, [state.news])

  // Get news count for a ticker
  const getNewsCount = useCallback((ticker: string): number => {
    return state.news[ticker]?.length || 0
  }, [state.news])
