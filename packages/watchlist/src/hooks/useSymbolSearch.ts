...
export function useSymbolSearch(): UseSymbolSearchReturn {
  ...
  */
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        if (hasExtension) {
          ...
          await searchViaExtension(trimmedQuery, requestId);
        } else {
          // Fallback to server API
          await searchViaServer(trimmedQuery, requestId);
        }
      } catch (error) {
  ...
  const searchViaExtension = async (query: string, requestId: string) => {
  ...
  const searchViaServer = async (query: string, requestId: string) => {
    ...
    try {
      ...
      let response = await fetch(`/api/tv/search?q=${encodeURIComponent(query)}`, {
        signal: controller.signal
      });
      
      // If TV proxy fails, fall back to FMP search
      if (!response.ok) {
        response = await fetch(`/api/symbols/search?q=${encodeURIComponent(query)}`, {
          signal: controller.signal
        });
      }
