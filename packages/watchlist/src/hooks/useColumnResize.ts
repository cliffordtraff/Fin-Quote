...
const mergeWithDefaults = (widths?: ColumnWidth | null): ColumnWidth => ({
  ...DEFAULT_WIDTHS,
  ...(widths ?? {})
})

const areColumnWidthsEqual = (a: ColumnWidth, b: ColumnWidth) => {
...
export function useColumnResize(
  ...
  const [columnWidths, setColumnWidths] = useState<ColumnWidth>(() => mergeWithDefaults(persistedWidths))
  ...
  // Start resizing
  const handleResizeStart = useCallback((e: React.MouseEvent, columnKey: string, columnIndex: number, edge: 'left' | 'right' = 'right') => {
    e.preventDefault()
    e.stopPropagation()
    ...
    handle.setPointerCapture((e.nativeEvent as PointerEvent).pointerId)
  ...
  const handleResizeEnd = useCallback((e: MouseEvent) => {
    ...
    if (activeHandleRef.current) {
      activeHandleRef.current.releasePointerCapture((e as PointerEvent).pointerId)
  ...
  const handleDoubleClick = useCallback((columnKey: string) => {
    ...
    if (columnIndex === -1) {
      ...
      if (baseIndex !== -1) {
        ...
        const hasControlColumn = firstHeader && (
    ...
    cells.forEach(cell => {
      ...
      if (!textContent.trim()) return // Skip empty cells
