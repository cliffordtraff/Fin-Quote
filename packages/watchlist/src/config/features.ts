...
export const features = {
  ...
  macroAttribution: {
    ...
    allowedSymbols: process.env.NEXT_PUBLIC_MACRO_ATTRIBUTION_SYMBOLS?.split(',').map(s => s.trim()),
...
export function shouldEnableServiceWorker(): boolean {
  ...
  // Feature flag check
  if (process.env.NEXT_PUBLIC_ENABLE_SW !== 'true') return false
 ...
 */
export function isMacroAttributionEnabled(symbol: string): boolean {
 ...
 */
function hashSymbolToBucket(symbol: string): number {
  let hash = 0
...
export function logFeatureFlagUsage(
  feature: 'macroAttribution' | 'serviceWorker',
  symbol: string,
  enabled: boolean
): void {
