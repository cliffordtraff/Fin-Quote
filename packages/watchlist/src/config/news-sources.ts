...
export function isBlockedSource(source: string): boolean {
  const normalizedSource = normalizeDomain(source);
  return BLOCKED_NEWS_SOURCES.some(blocked => 
    normalizedSource.includes(blocked.toLowerCase())
  );
}

// Get source quality tier
export function getSourceTier(source: string): 'tier1' | 'tier2' | 'press' | 'blocked' | 'unknown' {
  const normalizedSource = normalizeDomain(source);
...
export function sortBySourceQuality<T extends { site?: string }>(articles: T[]): T[] {
...
// Type definitions
export type NewsSource = typeof BLOCKED_NEWS_SOURCES[number] | 
                        typeof TIER_1_SOURCES[number] | 
                        typeof TIER_2_SOURCES[number] | 
                        typeof PRESS_RELEASE_SOURCES[number];
