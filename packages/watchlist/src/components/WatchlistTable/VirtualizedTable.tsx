...
interface VirtualizedTableProps {
  ...
  newsData?: { [symbol: string]: { count: number; loading?: boolean } }
  rssNewsData?: { [symbol: string]: { count: number; loading?: boolean } }
  analystData?: { [symbol: string]: { count: number; loading?: boolean } }
...
const VirtualizedTable = memo(function VirtualizedTable({
  ...
  deleteMode = false,
  reorderMode = false,
  containerHeight = 800, // Default height, can be overridden
  newsData = {},
  rssNewsData = {},
  analystData = {},
  onCellClick,
  ...
  const cellComponent = useCallback(({ columnIndex, rowIndex, style }: any) => {
        ...
        {columns.map((col, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Resize handle */}
              {index < columns.length - 1 && onResizeStart && (
                <span
                  className="col-resizer"
                  onPointerDown={(e) => onResizeStart(e as any, col.key, index)}
