...
interface TabManagerProps {
  ...
  isWatchlistModeActive?: boolean // Prevent tab reorder when watchlist is in delete/reorder mode
  onTabReorderModeChange?: (isActive: boolean) => void // Notify parent when tab reorder mode changes
  userHeader?: React.ReactNode // User header component to display
}
...
export default function TabManager({
  ...
  onTabsChange,
  onActiveTabChange,
  isWatchlistModeActive = false,
  onTabReorderModeChange,
  userHeader,
}: TabManagerProps) {
  const [editingIndex, setEditingIndex] = useState<number | null>(null)
  const [editValue, setEditValue] = useState('')
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; tabIndex: number } | null>(null)
  const [deleteConfirm, setDeleteConfirm] = useState<{ show: boolean; tabIndex: number; tabName: string } | null>(null)
  const [tabReorderMode, setTabReorderMode] = useState(false)
  const [draggedVisualIndex, setDraggedVisualIndex] = useState<number | null>(null) // Only for visual feedback
  
  // Use refs for drag state to avoid re-renders during drag
  const draggedTabIndexRef = useRef<number | null>(null)
  const dropTabIndexRef = useRef<number | null>(null)
  ...
  React.useEffect(() => {
    onTabReorderModeChange?.(tabReorderMode)
  }, [tabReorderMode, onTabReorderModeChange])

  return (
    <div className="watchlist-tabs-bar bg-watchlist-tab-bar" style={{
      display: 'flex',
      alignItems: 'flex-end',
      borderBottom: 'none',
      padding: '0 8px',
      ...
      {tabs.map((tab, index) => (
        <div
          key={index}
          className={`watchlist-tab ${index === activeTabIndex ? 'active bg-watchlist-tab-active' : 'bg-watchlist-tab-inactive'}`}
          style={{
            display: 'flex',
            alignItems: 'center',
            padding: '3px 8px',
          ...
          draggable={tabReorderMode}
          onClick={() => !tabReorderMode && onActiveTabChange(index)}
          onContextMenu={(e) => !tabReorderMode && handleContextMenu(e, index)}
          onDragStart={(e) => {
            if (tabReorderMode) {
          ...
          onDragEnd={() => tabReorderMode && handleTabDragEnd()}
        >
          {editingIndex === index ? (
            <input
              type="text"
              ...
              className="tab-rename-input border-watchlist-border text-watchlist-text-primary focus:ring-watchlist-focus-ring"
              style={{
      ...
      {tabReorderMode && (
        <ThemedButton
          ...
          variant="success"
          size="sm"
          title="Press Escape to exit reorder mode"
          className="ml-2"
        >
          Done
        </ThemedButton>
      ...
      {!tabReorderMode && tabs.length < MAX_TABS && editingIndex === null && (
        <button
          onClick={handleAddTab}
          className="add-tab-btn bg-watchlist-tab-bar hover:bg-watchlist-button-hover text-white"
          style={{
            display: 'flex',
            alignItems: 'center',
          ...
          title={`Add new watchlist (${tabs.length}/${MAX_TABS})`}
        >
          +
        </button>
      )}
      ...
      {/* User Header positioned at top right */}
      {userHeader && (
        <div style={{
          position: 'absolute',
          top: '8px',
      ...
      {deleteConfirm?.show && (
        <ThemedModal
          ...
          title={`Delete '${deleteConfirm.tabName}'?`}
        >
          <p className="text-watchlist-text-secondary mb-6">
