...
const defaultOptions: ExtendedHoursOverlayOptions = {
  color: 'rgba(173, 216, 230, 0.1)', // Light blue with low opacity
};

class ExtendedHoursOverlayRenderer implements IPrimitivePaneRenderer {
  _regions: Array<{ x1: number; x2: number }>;
  _color: string;

  constructor(regions: Array<{ x1: number; x2: number }>, color: string) {
...
class ExtendedHoursOverlayView implements IPrimitivePaneView {
  ...
  update() {
    ...
    const startTimestamp = typeof leftTime === 'number' ? leftTime : Math.floor(new Date(leftTime as string).getTime() / 1000);
    const endTimestamp = typeof rightTime === 'number' ? rightTime : Math.floor(new Date(rightTime as string).getTime() / 1000);
    ...
    while (currentTime <= endTimestamp) {
      if (this._isExtendedHours(currentTime)) {
        ...
        const x1 = timeScale.timeToCoordinate(regionStart as Time);
        const x2 = timeScale.timeToCoordinate(regionEnd as Time);
...
export class ExtendedHoursOverlay implements ISeriesPrimitive<Time> {
  _chart: IChartApi;
  ...
  constructor(
    ...
    options?: Partial<ExtendedHoursOverlayOptions>
  ) {
    this._chart = chart;
    this._series = series;
    this._options = {
