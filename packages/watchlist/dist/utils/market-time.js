 ...
 */
export function isMarketOpen(etTime = getCurrentETTime()) {
 ...
 */
export function getNextMarketOpen(etTime = getCurrentETTime()) {
    const dayOfWeek = etTime.getDay();
    const currentMinutes = etTime.getHours() * 60 + etTime.getMinutes();
    const marketOpenMinutes = MARKET_OPEN_HOUR * 60 + MARKET_OPEN_MINUTE;
    // Create a date for market open time
    let nextOpen = setMilliseconds(setSeconds(setMinutes(setHours(etTime, MARKET_OPEN_HOUR), MARKET_OPEN_MINUTE), 0), 0);
 ...
 */
export function getSmartCacheTTL(symbols, baselineTTL = 30000) {
    ...
    console.log(`[Smart Cache] Market CLOSED, caching until next open: ${hoursUntilOpen}h ${minutesUntilOpen}m`, `(${format(nextOpen, 'EEE h:mm a', { timeZone: ET_TIMEZONE })} ET)`);
    return msUntilOpen;
}
