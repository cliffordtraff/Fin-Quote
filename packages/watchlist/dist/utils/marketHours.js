...
export function isAfterHours() {
    ...
    if (day === 5 && timeInMinutes >= 960 && timeInMinutes < 1200)
        return true;
...
export function isForexOpen() {
    ...
    if (day === 5 && hours >= 17)
        return false; // Friday after 5 PM
    if (day === 0 && hours < 17)
        return false; // Sunday before 5 PM
    return true;
}
/**
 ...
 */
export function shouldPollAsset(symbol) {
 ...
 */
export function getPollingInterval(symbols = []) {
    // Check if any symbols are crypto (24/7 trading)
    const hasCrypto = symbols.some(s => s.includes('/') || s.endsWith('USD') || s.endsWith('USDT'));
    if (hasCrypto) {
...
export function formatMarketStatus(status) {
    ...
    if (status.nextOpenTime) {
        const hoursUntilOpen = Math.floor((status.nextOpenTime.getTime() - Date.now()) / (1000 * 60 * 60));
