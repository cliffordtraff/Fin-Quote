...
import { useDrawingTools } from '@watchlist/hooks/useDrawingTools';
 ...
 */
export function TradingViewChart({ symbol, timeframe = '15m', height = 500, showSMA20 = false, showSMA50 = false, showSMA200 = false, drawingTool = 'none', onDrawingComplete, onClearAll, onClose }) {
    ...
    useEffect(() => {
        if (onClearAll) {
            ...
            ;
            window.__chartClearAll = clearAllDrawings;
        }
    ...
    useEffect(() => {
        ...
        const chartData = data.map(candle => ({
            time: candle.time, // Cast to Time type for TypeScript
            open: candle.open,
            high: candle.high,
            low: candle.low,
            close: candle.close,
        ...
        if (chartRef.current && chartData.length > 0) {
            ...
            const twoYearsAgo = latestTime - (2 * 365 * 24 * 60 * 60);
    ...
    useEffect(() => {
        ...
        if (showSMA20 && data.length >= 20)
            periodsToCalculate.push(20);
        if (showSMA50 && data.length >= 50)
            periodsToCalculate.push(50);
        if (showSMA200 && data.length >= 200)
            periodsToCalculate.push(200);
    ...
    return (_jsx("div", { className: "relative w-full", children: _jsxs("div", { className: "relative", children: [loading && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-white dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-75 z-10", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto" }), _jsx("p", { className: "mt-2 text-sm text-gray-600 dark:text-gray-400", children: "Loading chart..." })] }) })), error && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-white dark:bg-gray-900 z-10", children: _jsxs("div", { className: "text-center text-red-600 dark:text-red-400 p-4", children: [_jsx("p", { className: "font-semibold", children: "Failed to load chart" }), _jsx("p", { className: "text-sm mt-1", children: error.message })] }) })), _jsx("div", { ref: chartContainerRef, className: "w-full", style: { height: `${height}px` } })] }) }));
}
