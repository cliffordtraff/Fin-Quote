...
/* ───────── Aggregate 1-min candles to 5-min OHLC bars ───────── */
...
function aggregateTo5Min(candles: DayCandle[]): Candle5Min[] {
  ...
  for (const c of candles) {
    // c.date is like "2026-03-12 09:30:00" — extract minute, floor to 5
...
/* ───────── Ghost day candles canvas (Approach #4) ───────── */

function GhostDayCandlesCanvas({
  candles,
  previousClose,
  chartHeight,
  padding,
  ...
  useEffect(() => {
    ...
    if (previousClose !== null && previousClose >= yMin && previousClose <= yMax) {
      ...
      ctx.save()
      ctx.setLineDash([4, 4])
      ctx.strokeStyle = theme === 'dark' ? 'rgba(156,163,175,0.25)' : 'rgba(107,114,128,0.25)'
      ctx.lineWidth = 1
      ctx.beginPath()
    ...
    for (let i = 0; i < bars.length; i++) {
      ...
      const green = theme === 'dark' ? `rgba(74,222,128,${opacity})` : `rgba(34,197,94,${opacity})`
      const red = theme === 'dark' ? `rgba(248,113,113,${opacity})` : `rgba(239,68,68,${opacity})`
...
interface PulseCardProps {
  ...
  /** 1-min day candles to render as ghosted background bars */
  ghostCandles?: DayCandle[]
  /** Previous close for ghost layer dashed line */
  ghostPreviousClose?: number | null
}

function PulseCard({ symbol, stream, chartData, theme, timeframe, exaggerate, dayHigh, dayLow, label, color = '#22c55e', lineOnly = false, windowOverride, ghostCandles, ghostPreviousClose }: PulseCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden relative">
      ...
      <div style={{ height: chartHeight, position: 'relative' }}>
        {ghostCandles && ghostCandles.length > 0 && (
          <GhostDayCandlesCanvas
            candles={ghostCandles}
            previousClose={ghostPreviousClose ?? null}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <PulseCard
        ...
        ghostCandles={dayData?.candles}
        ghostPreviousClose={dayData?.previousClose}
      />
      <PulseCard
        symbol={symbol}
        stream={stream}
...
export default function PulseLabDashboard() {
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Pulse Lab</h1>
