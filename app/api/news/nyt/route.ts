...
const NYT_FEEDS = {
  ...
  markets: 'https://rss.nytimes.com/services/xml/rss/nyt/YourMoney.xml',
...
export async function GET(request: NextRequest) {
  try {
    ...
    const feedType = searchParams.get('feed') || 'business'
    const ifNoneMatch = request.headers.get('if-none-match')

    const cacheKey = `nyt_${feedType}`
    ...
    const feedUrl = NYT_FEEDS[feedType as keyof typeof NYT_FEEDS] || NYT_FEEDS.business
    ...
    for (const item of feed.items || []) {
      ...
      const author = item.author || (item as any).creator || ''
        ...
        categories: categories
          .map((cat: any) => {
            ...
            if (cat && typeof cat === 'object') {
              return cat._ || cat.text || cat.name || (cat['$'] && cat['$'].term) || ''
            }
            return ''
          })
          .filter(Boolean),
        feedTopic: getFeedTopic('NYT', feedType),
    ...
    console.error('Error fetching NYT RSS:', error)
